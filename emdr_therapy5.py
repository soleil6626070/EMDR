...
def get_next_processing_filename():
    """Get the next available processing responses filename"""
    counter = 1
    while True:
        filename = os.path.join(PROCESSING_RESPONSES_DIR, f"Processing_Session_{counter}.json")
        if not os.path.exists(filename):
            return filename
        counter += 1
...
class EMDRProgram:
    def __init__(self):
        ...
        # Initialize audio recorder with whisper model size (you can change this)
        # Options: "tiny", "base", "small", "medium", "large"
        self.audio_recorder = AudioRecorder(whisper_model_size="base")
        self.llm_handler = LLMHandler()
        
        # Initialize program state - start with menu
        self.current_cycle = 0
        self.state = "menu"  # States: menu, target_identification, recording, transcribing, target_selection, cue_in, oscillating, waiting, processing_recording, processing_transcribing, feedback, fading
        ...
        # Processing responses linked list
    ...
    def handle_processing_transcribing_state(self):
        """Handle transcribing during processing phase"""
        self.draw_transcribing()
    ...
    def handle_fading_state(self):
        ...
        if fade_progress >= 1.0:
            ...
            if self.current_cycle >= TOTAL_CYCLES:
                # All cycles complete, save processing responses and return to menu
                print("All cycles complete. Saving processing responses...")
                filename = get_next_processing_filename()
                if self.processing_responses.save_to_file(filename):
                    print(f"Processing responses saved to {filename}")
    ...
    def process_transcription_complete(self, transcription):
        ...
        elif self.state == "processing_transcribing":
            # Processing phase - save to linked list
            self.processing_responses.add_response(self.current_cycle + 1, transcription)
            print(f"Processing response for cycle {self.current_cycle + 1}: {transcription}")
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                # Handle processing recording stop
                    ...
                    audio_file = self.audio_recorder.stop_recording()
                    if audio_file:
                        self.state = "processing_transcribing"
                        # Start transcription in a separate thread
                        threading.Thread(
                            target=self._transcribe_audio,
                            args=(audio_file,)
    ...
    def run(self):
        ...
        while running:
            ...
            if self.state == "menu":
                ...
                self.handle_processing_transcribing_state()
